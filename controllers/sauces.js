  ...
  switch (req.body.like) {
    ...
    case -1:
      Sauce.updateOne(
        ...
        .then(() => res.status(201).json({message : `Trop piquant j'imagine ?`}))
        .catch((error) => res.status(400).json({ error }));
    ...
    case 1:
      Sauce.updateOne(
        ...
        .then((sauce) => res.status(201).json({message : `Merci d'aimer cette sauce !`}))
        .catch((error) => res.status(400).json({ error }));
        console.log(`Merci d'aimer cette sauce !`)
      break;
      ...
      Sauce.findOne({ _id: req.params.id }).then((sauce) => {
        ...
        if (sauce.usersLiked.includes(req.body.userId)) {
          Sauce.updateOne(
            ...
            .then(() => res.status(201).json({ message: `Vous changez d'avis...dommage` }))
            .catch((error) => res.status(400).json({ error }));
        ...
        //...il retire son dislike :
          ...
          Sauce.updateOne(
            ...
            .then(() => res.status(201).json({ message: `Il fallait juste le temps de s'y habituer` }))
            .catch((error) => res.status(400).json({ error }));
